...
// Which providers have credentials configured (drives the "Live" badge + whether we
// build a prefilled link vs. hand off to the public page). Coinbase needs a session
// token exchanged server-side (a separate integration), so it's never auto-"live" here.
...
function build(provider: string, ctx: Ctx): string | null {
  ...
  switch (provider) {
    case 'moonpay': {
      ...
      return 'https://sell.moonpay.com' + search;
    }
    case 'transak': {
    ...
    case 'ramp': {
      ...
      return 'https://app.ramp.network/?' + p.toString();
    }
    case 'banxa': {
      ...
      return 'https://' + c.banxa.subdomain + '.banxa.com/?' + p.toString();
    }
    case 'mercuryo': {
    ...
    case 'kado': {
      ...
      return 'https://app.kado.money/?' + p.toString();
    }
    case 'unlimit': {
      ...
      if (amt) p.set('amount', amt);
      return 'https://crypto.unlimit.com/?' + p.toString();
    }
    default: return null;
  }
}
...
router.get('/v1/offramp/providers', (_req: Request, res: Response) => {
  ...
  return res.json({ configured: configuredMap() });
});
...
router.post('/v1/offramp/link', linkLimiter, (req: Request, res: Response) => {
  ...
  // Not configured → hand off to the provider's public page so the flow still works.
  return res.json({ url: url || HOMES[provider], configured: !!url });
});
